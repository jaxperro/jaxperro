...
- `world/js/main.js` — entry point: wires scene, ocean, boat, islands, controls, damped third-person follow camera, and the render loop. The page loads in an intro mode (camera facing the bow, boat locked, Explore/Regular buttons); Explore sweeps the camera around the boat into the chase position and enables the joystick, Regular navigates to the home page. Owns the island action map (e.g. `catPopup` reuses the home page's `createPopupManager` + `ensureCatAnimation`), gates input while a popup is open, and intercepts island taps in a capture-phase listener so they never reach the joystick. Exposes `window.__world` for console debugging.
