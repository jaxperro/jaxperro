...
- `world/js/boat.js` — low-poly boat built from primitives plus arcade physics (throttle/drag/speed-scaled steering) and wave-sampled buoyancy (height, pitch, roll). If `BOAT.model` is set it loads that model and swaps it in for the procedural visual; the group stays the physics target so driving is untouched.
- `world/js/controls.js` — merges WASD/arrow keys and the nipplejs joystick into one `{ throttle, steer }` input.
- `world/js/models.js` — shared `GLTFLoader` wrapper. `loadModel(url, { scale, yOffset, rotationY })` resolves with a model root; `clearGroup(group)` disposes a procedural placeholder before a swap. Models live in `world/assets/models/` (see its README).
- `world/js/islands.js` — portfolio islands, built from the `ISLANDS` list in `config.js`. Each entry is pure data (position, footprint, interaction radius, action, optional `model`). Each island is a wrapper Group holding its visual (a procedural box by default, or a swapped-in model); the press animation scales the wrapper and tints every material it contains, so a model needs no special handling. The module owns proximity arming (pulse + glow affordance), the tap press animation (shrink/darken/recover, then the action fires), and the axis-aligned colliders (always from `size`, never the model) the boat resolves against.
- `world/js/main.js` — entry point: wires scene, ocean, boat, islands, controls, damped third-person follow camera, and the render loop. The page loads in an intro mode (camera facing the bow, boat locked, Explore/Regular buttons); Explore sweeps the camera around the boat into the chase position and enables the joystick, Regular navigates to the home page. Owns the island action map (`catPopup` reuses the home page's `createPopupManager` + `ensureCatAnimation`; `emptyPopup` shows a content-less placeholder card), gates input while a popup is open, and intercepts island taps in a capture-phase listener so they never reach the joystick. Exposes `window.__world` for console debugging.
- `world/assets/models/` — drop-in `.glb`/`.gltf` models for the boat and islands; `README.md` documents the config workflow.

**Adding a portfolio island** = add an entry to `ISLANDS` in `world/js/config.js`; if it needs a new behavior, add an action type to the map in `main.js`. New islands default to `emptyPopup` (a placeholder card) until given bespoke content. The world page reuses the home popup (`styles/components/popup.css`, `scripts/ui/popups.js`, `scripts/ui/riveCat.js`); those modules now use root-absolute asset paths so they work from any page depth.

**Swapping a 3D model** = drop a `.glb` in `world/assets/models/` and point `BOAT.model` or an island's `model` field at it (`{ url, scale, yOffset, rotationY }`); `null` keeps the procedural shape. A failed load logs a warning and falls back to the procedural mesh.
